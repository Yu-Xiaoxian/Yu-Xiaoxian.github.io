...
除参数表和函数体以外，构造还可以拥有初始化列表，效率比在函数体中赋值要高一些。当传递参数是类时，在初始化列表中进行初始化，只需要执行一次复制构造函数。
...
public 和 protected 成员，访问权限保持不变

private成员，不可以直接访问，但可以通过公有函数接口访问

### 私有继承

继承的所有成员都变成了私有成员，只有派生类自己定义的成员才能够被对象访问。

### 保护继承

公有成员变成了保护成员。对象不可以直接访问基类的保护成员，但派生类可以访问。对于派生类而言，表现如同公有。但对于对象而言，表现如同私有。
